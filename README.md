# **White Paper & Documentation For PointGun**
<!-- Roles: -->
> Documentation | Advisor For QA | Administrator
<hr>

#### A Blockchain Lab developing a GTA inspired P2E game deployed through the *Matic* ecosystem on *Polygon.*
<hr>

```
-----BEGIN PGP PUBLIC KEY BLOCK-----


6sYeS2gA/0FBWfNmkj2ZjhMmJIf55Xvc30737XUiauiB901nrVQD
=KnH6
-----END PGP PUBLIC KEY BLOCK-----
```
